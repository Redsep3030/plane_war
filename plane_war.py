...
rock_images = []
for a in range(7):
    rock_images.append(pygame.image.load(os.path.join(folder,'img',f'rock{a}.png')).convert())
...
expl_anim['player'] = []
for c in range(9):
    expl_img = pygame.image.load(os.path.join(folder,'img',f'expl{c}.png')).convert()
    expl_img.set_colorkey(BLACK)
    expl_anim['lg'].append(pygame.transform.scale(expl_img,(75,75)))
    expl_anim['sm'].append(pygame.transform.scale(expl_img,(30,30)))
    player_expl_img = pygame.image.load(os.path.join(folder,'img',f'player_expl{c}.png')).convert()
